...
class Annotater:
    ...
    def setup_gui(self):
        ...
        #self.canvas.bind('<Double-Button-1>', lambda e: self.title_box(e))
        ...
        for c in list(self.colors.keys()):
            for mb in self.boxids[c][::-1]:
                ...
                if x0 <= event.x < x1 and\
                   ...
                   self.segtypes[mb] = 'title' if self.segtypes[mb]=='text' else 'title'
                   self.canvas.itemconfig(mb, stipple='gray75' if self.segtypes[mb]=='text' else 'gray25')
                   return True
        return False
    ...
    def load(self):
        ...
        for s in S.segments:
            stipple = 'gray75' if s.segtype=='text' else 'gray25'
            for b in s.boxes:
                print(b.coors)
                self.boxids[self.curr_colkey].append(
                   self.canvas.create_rectangle(b.coors[0][1],b.coors[0][0],b.coors[1][1],b.coors[1][0],
                                                fill=self.colors[self.curr_colkey], stipple=stipple)
                )
            i += 1
            c = y[i]
            self.colors[c] = mycolors[c]
