...
public:
    ...
    vector<float> join_hori(vector<vector<float>>& coors){
        vector<vector<float>> ret;
        vector<float> tmp;
        for(int i = 0;i < coors[0].size();i++){
            tmp.clear();
            for(int j = 0;j < coors.size();j++){
                tmp.push_back(coors[j][i]);
            }
            ret.push_back(tmp);
        }
        tmp.clear();
    ...
    vector<float> join_verti(vector<vector<float>>& coors){
        vector<float> ret;
        if(coors.empty())
    ...
    void parse(const char* scrapedname, vector<vector<string>>& contents, vector<vector<float>>& coors, vector<float>& height){
    ...
    float findtitleheight(vector<float> heights, float bins){
        float m = *min_element(heights.begin(), heights.end());
        float M = *max_element(heights.begin(), heights.end());
        float r = M - m;
        float step = r/bins;
    ...
    void getstrips(vector<vector<float>>& coors, vector<float>& height, vector<vector<float>> &textcoors, vector<vector<float>> &titlecoors){
    ...
    void gettitleblocks(vector<vector<float>> &titlecoors){
        ...
        for(int it = 0;it < num_titles;it++){
            ...
            for(int it0 = it+1;it0 < num_titles;it0++){
                if(flag[it0])
                    continue;
                if(adjoins(titlecoors[it], titlecoors[it0])){
                    flag[it0] = true;
                    todel.push_back(it0);
                    titlecoors[it] = join_hori2(titlecoors[it], titlecoors[it0]);
                }
            }
        }
        for(auto i:todel)
            titlecoors.erase(titlecoors.begin() + i - 1);
    }
    ...
    vector<vector<vector<float>>> assign_textblocks(vector<vector<float>>& titlecoors, vector<vector<float>>& textcoors){
        ...
        for(auto i:textcoors){
            ...
            if(max_label > 0)
                ...
                cout<<"no title dominates [%f, %f, %f, %f]"<<i[0]<<i[1]<<i[2]<<i[3]<<endl;
            max_label = -1;
        }
        return ret;
    }
    
    vector<vector<vector<float>>> group_textblocks(vector<vector<vector<float>>> assignments){
        long title_number = assignments.size();
        vector<vector<vector<float>>> articleblocks(title_number, vector<vector<float>>());
        for(int i = 0; i < title_number; i++){
            while(!assignments[i].empty()){
                ...
                articleblocks[i].push_back(join_verti(ws));
