...
class Polygon:
    ...
    def area(self): #TODO: memoize!
        return sum(b.area(Polygon.weight_image) for b in self.boxes)
    def intersect(self, other):
        intersects = (bs.intersect(bo) for bs in self.boxes for bo in other.boxes)
        return Polygon(i for i in intersects if i.area())
    def __str__(self):
    ...
    jaccard_precision = create_jaccard(lambda s,t,i: s.area(),
       docstring='''returns proportion of self's area also in truth's area''')
    jaccard_recall = create_jaccard(lambda s,t,i: t.area(),
       docstring='''returns proportion of truth's area also in self's area''')
    jaccard_similarity = create_jaccard(lambda s,t,i: s.area()+t.area()-i,
