...
class EndToEnd:
    ...
    # @profile(stream = fp)        
    def segment(self):
        if os.path.isfile(self.seg_out_path+'/'+'TBnewtime'):
            os.system('rm %s'%(self.seg_out_path,)+'/'+'TBnewtime')
        if os.path.isfile(self.seg_out_path+'/'+'TBoldtime'):
            os.system('rm %s'%(self.seg_out_path,)+'/'+'TBoldtime')
        if os.path.isfile(self.seg_out_path+'/'+'VDoldtime'):
            os.system('rm %s'%(self.seg_out_path,)+'/'+'VDoldtime')
        if os.path.isfile(self.seg_out_path+'/'+'VD1oldtime'):
            os.system('rm %s'%(self.seg_out_path,)+'/'+'VD1oldtime')      
        if os.path.isfile(self.seg_out_path+'/'+'VD2oldtime'):
            os.system('rm %s'%(self.seg_out_path,)+'/'+'VD2oldtime')  
        if os.path.isfile(self.seg_out_path+'/'+'VD2newtime'):
            os.system('rm %s'%(self.seg_out_path,)+'/'+'VD2newtime')                   
    ...
    def collect_data(self):
        ...
        for i, imp_name in enumerate(self.implementations):
            ...
            global tmp
            tmp = 0
            if(imp_name == "textblocksBS.py"):
                with open("../../output/segment/TBoldtime") as f:
                    for i in f.read().rstrip().split(' '):
                        tmp += float(i)
                    alg_result['time'] = tmp
                f.close()
            elif(imp_name == "textblocksBSNew.py"):
                tmp = 0
                with open("../../output/segment/TBnewtime") as f:
                    for i in f.read().rstrip().split(' '):
                        tmp += float(i)
                    alg_result['time'] = tmp
                f.close()
            elif(imp_name == "VerticalDominance.py"):
                tmp = 0
                with open("../../output/segment/VDoldtime") as f:
                    for i in f.read().rstrip().split(' '):
                        tmp += float(i)
                    alg_result['time'] = tmp                
                f.close() 
            elif(imp_name == "VerticalDominanceNew.py"):
                tmp = 0
                with open("../../output/segment/VDnewtime") as f:
                    for i in f.read().rstrip().split(' '):
                        tmp += float(i)
                    alg_result['time'] = tmp         
                f.close()     
            elif(imp_name == "VerticalDominance1New.py"):
                tmp = 0
                with open("../../output/segment/VD1newtime") as f:
                    for i in f.read().rstrip().split(' '):
                        tmp += float(i)
                    alg_result['time'] = tmp         
                f.close()                           
            elif(imp_name == "VerticalDominance1.py"):
                tmp = 0
                with open("../../output/segment/VD1oldtime") as f:
                    for i in f.read().rstrip().split(' '):
                        tmp += float(i)
                    alg_result['time'] = tmp         
                f.close()
